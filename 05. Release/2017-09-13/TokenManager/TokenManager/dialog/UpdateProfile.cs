...
namespace TokenManager.dialog
{
    public partial class UpdateProfile : Form
    {
        ...
        private bool IsValidInputValue()
        {
            ...
            try
            {
                isValidEmail = Regex.IsMatch(Email, TokenManagerConstants.EMAIL_REGEX, RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(250));
            }
            catch (RegexMatchTimeoutException)
            {
                LOG.Error("Cannot check email address regex");
                isValidEmail = false;
            }
            if (!isValidEmail)
            {
            ...
            {
                LOG.Error("Cannot check phone number regex");
                isValidPhone = false;
            }
            if (!isValidPhone)
            {
