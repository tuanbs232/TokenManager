...
namespace TokenManager.tmsclient
{
    class TMSClient
    {
        ...
        private const int PROCESS_TIME = 15000;
        //TEST-------------------------------------------------------------------------------
        ...
        //Base URI
        const string ENPOINT_URL = @"http://10.145.27.22/tms/";
        ...
        const string UPDATE_PROFILE_RESOURCE = "restapi/token/email_phone";
        ...
        public static int QueryRenewCert(CustomerInfo info)
        {
            ...
            try
            {
                Content = HandleQuery(pramJson, RENEW_CERT_RESOURCE);
            }
        ...
        public static int QueryUnlockPin(CustomerInfo info)
        {
            ...
            try
            {
                Content = HandleQuery(pramJson, QUERY_UNLOCK_PIN_RESOURCE);
            }
            catch (TokenManagerException ex)
            {
        ...
        public static int UnlockPin(CustomerInfo info)
        {
            ...
            try
            {
                Content = HandleQuery(pramJson, UNLOCK_PIN_RESOURCE);
            }
            catch (TokenManagerException ex)
            {
        ...
        public static int UpdateProfile(CustomerInfo info)
        {
            ...
            try
            {
                Content = HandleQuery(pramJson, UPDATE_PROFILE_RESOURCE);
            }
            catch (TokenManagerException ex)
            {
        ...
        /// <returns></returns>
        private static string HandleQuery(string param, string resouce)
        {
        ...
        public static string GetErrorMessage(int code)
        {
            switch (code)
            {
                case TMSClient.CUSTOMER_INFO_NULL:
                    return "Lỗi khi lấy thông tin token";

                case TMSClient.CANNOT_CONNECT_TO_TMS:
                    return "Không thể kết nối đến máy chủ TMS";

                case TMSClient.SERVER_NOT_RESPONSE:
                    return "Hệ thống TMS không phản hồi, liên hệ với quản trị để được hỗ trợ";

                case TMSClient.CANNOT_PARSE_TMS_RESPONSE:
                    return "Không thể phân tích dữ liệu TMS";
